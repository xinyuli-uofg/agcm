...
def wrap_pretrained_model(c_extractor_arch, pretrain_model=True, x2c_used_pretrain=False, x2c_used_pretrain_path='', architecture=None):
    def _result_x2c_fun(output_dim, architecture):

        if x2c_used_pretrain:
            if x2c_used_pretrain_path=='./ckpt/backbone.pth':
                if architecture == 'Attention_ConceptEmbeddingModel' or architecture == 'Attention_ConceptEmbeddingModel_Reg' or architecture == 'Attention_ConceptEmbeddingModel_ROI' or architecture == 'Attention_ConceptEmbeddingModel_Reg_ROI':
                    ...
                    checkpoint = torch.load(x2c_used_pretrain_path, map_location='cpu')
...
class WrapperModule(pl.LightningModule):
    ...
    def configure_optimizers(self):
        print("optimizer", self.optimizer_name.lower())
